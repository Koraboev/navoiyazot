    ...
    <section class="pt-4 pb-3" style="background-color: #f8f9fa">
        <div class="container">
            <div class="section-title wow fadeIn" data-wow-delay="200ms">
                <p class="mb-2"><a href="/">{{ trans('Home') }}</a> > <a href="{{ url(app()->getLocale().'/posts') }}">{{ trans('news') }}</a> > {{ $post->title }}</p>
            </div>
            <div class="row g-5 mt-2">
                ...
                <div class="col-md-4 px-4 py-1 mt-2">
                    <div class="bg-white rounded-2 p-4 pb-1">
                        <h4 class="post__recent__post__title pb-2 display-25 position-relative">{{ trans('Latest news') }}</h4>
                        @foreach($posts as $item)
                            @if($item->id != $post->id)
                            <a href="{{ url(app()->getLocale().'/posts/'.$item->slug) }}">
                                <div class="post__recent__post__card row mt-3 mb-4">
                                    <img class="col-3 h-100 px-2 post__recent__post__card__img" src="{{ asset('storage/'.$item->image[0]) }}" alt="...">
                                    <div class="col-9">
                                        <p class="lh-base mb-1">{{ Str::limit($item->title, 50) }}</p>
